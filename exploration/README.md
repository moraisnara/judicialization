...
|---|---|---|
| `03_estimation/01c_patch_family_ivs.py` | Patches the `bartik_iv_{family}`, `delta_log1p_{family}_2024_2020` and top-topic `share_{code}_2020` columns into `data/estimation/executive_margin_design.csv` | Producer for `03_family_iv.R` alone: nothing under `code/` reads the columns it adds. It augments the shared design in place rather than writing its own artifact, so its paths are unchanged by the move |
| `03_estimation/03_family_iv.R` | IV split by lawsuit family (4 families x outcomes) | `family_iv_results.csv` is cited by no document and read by no script. Superseded by `13_reclassification_robustness.R` (family-level Z) and `11_summary_indices.R`. **Run order: `01c_patch_family_ivs.py` first** — it adds the `bartik_iv_{family}` columns to `data/estimation/executive_margin_design.csv`, which the committed vintage of that file does not carry, which is why `03_family_iv.R` otherwise exits on its column guard |

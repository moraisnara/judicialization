...
write_pretrend_tex <- function(keys, path, caption_hint) {
  keys <- keys[keys %in% names(rf_fit)]
  mods <- rf_fit[keys]
  K    <- length(mods)
  hdr  <- sprintf("\\textbf{%s}", ALL_LAB[keys])
  coef_cells <- vapply(mods, function(m)
    sprintf("%.3f%s", coef(m)[INSTRUMENT], hb_star(pvalue(m)[INSTRUMENT])), "")
  se_cells   <- vapply(mods, function(m)
    sprintf("\\textcolor{mygray}{(%.3f)}", se(m)[INSTRUMENT]), "")
  n_cells    <- vapply(mods, function(m)
    formatC(nobs(m), format = "d", big.mark = ","), "")
  mean_cells <- vapply(keys, function(k)
    sprintf("%.3f", res$pretrend_mean[match(k, res$outcome)]), "")
  lines <- c(
    sprintf("%% Auto-generated by code/03_estimation/05_pretrend_balance.R (%s)", caption_hint),
    ...
    "\\toprule\\toprule",
    hb_row("Dep.\\ var.\\ (pre-trend, 2016$\\to$2020):", hdr),
    "\\midrule",
    "\\rowcolor{mylight}",
    hb_row("\\textbf{Instrument ($Z$)}", coef_cells),
    "\\rowcolor{mylight}",
    hb_row("", se_cells),
    "\\midrule",
    hb_row("$N$", n_cells),
    hb_row("Pre-trend mean", mean_cells),

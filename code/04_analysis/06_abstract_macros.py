...
ANCOVA_LEVEL = {
    ...
    'delta_effective_n_candidates_vote_2024_2020':   'effective_n_candidates_vote_2024',
    'delta_vote_hhi_candidate_2024_2020':            'vote_hhi_candidate_2024',
    'delta_top2_vote_share_2024_2020':               'top2_vote_share_2024',
}
...
# IV — field concentration ladder (winner-take-more consolidation)
r_effn   = get_iv('baseline', 'delta_effective_n_candidates_vote_2024_2020')
r_hhi    = get_iv('baseline', 'delta_vote_hhi_candidate_2024_2020')
r_top2   = get_iv('baseline', 'delta_top2_vote_share_2024_2020')

# IV — pre-trend falsification (2020-2016 placebo on instrumented treatment)
r_pt_margin = get_iv('baseline', 'pretrend_margin_top1_top2_2020_2016')
r_pt_effn   = get_iv('baseline', 'pretrend_effective_n_candidates_vote_2020_2016')
r_pt_hhi    = get_iv('baseline', 'pretrend_vote_hhi_candidate_2020_2016')
r_pt_top2   = get_iv('baseline', 'pretrend_top2_vote_share_2020_2016')
...
# Worked-example derived numbers (app:readtau) -- routed through macros so the
# prose tracks the estimate. Per-doubling effect = log(2) * tau, in pp; the
# reduced-form pp per unit of Z = first-stage slope * tau (2SLS), in pp.
M['MarginDoublePP']    = f1(0.693147 * r_margin['coef'] * 100)
M['MarginRFperUnitPP'] = f1(fs_base['coef'] * r_margin['coef'] * 100)
...
# --- Field concentration ladder (winner-take-more) ---
# eff-N candidates is a count (negative = field thins); HHI and top-2 share are
# 0-1 indices (positive = vote concentrates). The pre-trend twins are the
# 2020-2016 placebo: clean for top-2, fails for eff-N/HHI/margin (the same
# convergence property that the headline inherits — reported openly).
M['EffNCoef'] = coef(r_effn['coef']); M['EffNSE'] = se_par(r_effn['se']); M['EffNP'] = pval(r_effn['p'])
M['EffNTF']   = tick_cross(r_effn['reject_tF_5pct'])
M['HHICoef']  = coef(r_hhi['coef']);  M['HHISE']  = se_par(r_hhi['se']);  M['HHIP']  = pval(r_hhi['p'])
M['HHITF']    = tick_cross(r_hhi['reject_tF_5pct'])
M['TopTwoCoef'] = coef(r_top2['coef']); M['TopTwoSE'] = se_par(r_top2['se']); M['TopTwoP'] = pval(r_top2['p'])
M['TopTwoTF']   = tick_cross(r_top2['reject_tF_5pct'])

# Pre-trend falsification (placebo coefficients; want ~0).
M['PtMarginCoef'] = coef(r_pt_margin['coef']); M['PtMarginSE'] = se_par(r_pt_margin['se']); M['PtMarginP'] = pval(r_pt_margin['p'])
M['PtEffNCoef']   = coef(r_pt_effn['coef']);   M['PtEffNSE']   = se_par(r_pt_effn['se']);   M['PtEffNP']   = pval(r_pt_effn['p'])
M['PtHHICoef']    = coef(r_pt_hhi['coef']);    M['PtHHISE']    = se_par(r_pt_hhi['se']);    M['PtHHIP']    = pval(r_pt_hhi['p'])
M['PtTopTwoCoef'] = coef(r_pt_top2['coef']);   M['PtTopTwoSE'] = se_par(r_pt_top2['se']);   M['PtTopTwoP'] = pval(r_pt_top2['p'])
...
# Every focus outcome, exposure-robust: the AKM(2019)/BHJ(2022) shock-clustered SE,
# its ratio to the conventional cluster-robust SE (how much the few-shocks structure
# inflates the SE), the BHJ p-value, the null-imposed AKM0 confidence interval, and
# whether that CI excludes zero. This lets the appendix report exposure-robust
# inference entirely from macros -- nothing hand-typed. Genuine result: the margin SE
# inflates only ~6% (1.06x), NOT the retracted ~9x heuristic, and the AKM0 CI still
# excludes zero for the margin, winner, runner-up and mayoral blank rate.
_ER_TAGS = {
    'delta_margin_top1_top2_2024_2020':    'ERMargin',
    'delta_winner_vote_share_2024_2020':   'ERWinner',
    'delta_runnerup_vote_share_2024_2020': 'ERRunnerUp',
    'delta_winner_majority_2024_2020':     'ERWinMaj',
    'delta_female_vote_share_2024_2020':   'ERFemaleVS',
    'delta_blank_rate_2024_2020':          'ERBlank',
}
for _oc, _tag in _ER_TAGS.items():
    _row = er.loc[(er['outcome'] == _oc) & (er['spec'] == 'baseline') &
                  (er['variant'] == 'adversarial')]
    if _row.empty:
        continue
    _row = _row.iloc[0]
    M[f'{_tag}SEconv'] = f'{_row["se_conventional"]:.3f}'
    M[f'{_tag}SEbhj']  = f'{_row["se_bhj"]:.3f}'
    M[f'{_tag}Ratio']  = f'{_row["se_bhj"] / _row["se_conventional"]:.2f}'
    M[f'{_tag}Pbhj']   = pval(_row['p_bhj'])
    M[f'{_tag}CI']     = f'[{_row["akm0_ci_low"]:+.3f}, {_row["akm0_ci_high"]:+.3f}]'
    M[f'{_tag}Excl']   = r'\tick' if (_row['akm0_ci_low'] > 0 or _row['akm0_ci_high'] < 0) else r'\cross'
...
_PERSD_SHARE = {
    ...
    'HHI': r_hhi, 'TopTwo': r_top2,
}
for tag, r in _PERSD_SHARE.items():
    M[f'{tag}PerSD']       = per_sd_pp(r['coef'])
    M[f'{tag}PerSDSigned'] = per_sd_pp_signed(r['coef'])

# Eff-N candidates is a raw count, not a share: per-SD effect stays in candidate
# units (signed; +1 SD of D shaves ~|tau*SD| effective candidates off the field).
M['EffNPerSDSigned'] = f'{r_effn["coef"] * TREAT_SD:+.3f}'
M['EffNPerSD']       = f'{abs(r_effn["coef"] * TREAT_SD):.3f}'
...
# Concentration ladder: 2024-level mean (ANCOVA); pretrend twins: pretrend mean.
M['EffNMean']   = m3(repmean(panel_ex, 'delta_effective_n_candidates_vote_2024_2020'))
M['HHIMean']    = m3(repmean(panel_ex, 'delta_vote_hhi_candidate_2024_2020'))
M['TopTwoMean'] = m3(repmean(panel_ex, 'delta_top2_vote_share_2024_2020'))
M['PtMarginMean'] = m3(dvmean(panel_ex, 'pretrend_margin_top1_top2_2020_2016'))
M['PtEffNMean']   = m3(dvmean(panel_ex, 'pretrend_effective_n_candidates_vote_2020_2016'))
M['PtHHIMean']    = m3(dvmean(panel_ex, 'pretrend_vote_hhi_candidate_2020_2016'))
M['PtTopTwoMean'] = m3(dvmean(panel_ex, 'pretrend_top2_vote_share_2020_2016'))
...
N_SUBJECTS_KEPT = shiftd['topic_code'].nunique()
...
# ── Rotemberg weights: which few shocks identify the design ───────────────────
# Goldsmith-Pinkham/Sorkin/Swift decomposition. The alpha_k sum to 1 across all
# subjects, so alpha_pct is each subject's share of net identifying weight; a few
# subjects carry it (that concentration IS the design's binding limit). We surface
# the top-weighted subjects, their share, and how propaganda-dominated they are —
# every number from the committed rotemberg_weights.csv, nothing hand-typed.
NUM_WORDS = ['One', 'Two', 'Three', 'Four', 'Five']

def rot_label(name):
    """Shortest informative label: drop the generic 'Propaganda Politica -
    Propaganda Eleitoral -' vehicle prefix, keep the specific subject; escape
    LaTeX-special '&', cap length so the frame table stays on one line."""
    seg = str(name).split(' - ')[-1].strip()
    if len(seg) > 26:
        seg = seg[:25].rstrip() + r'\,\ldots'
    return seg.replace('&', r'\&')

try:
    rot = pd.read_csv(os.path.join(DESC, 'rotemberg_weights.csv'))
    rot_pos = rot[rot['alpha'] > 0].sort_values('alpha', ascending=False).reset_index(drop=True)
    top5 = rot_pos.head(5)
    for i, (_, rw) in enumerate(top5.iterrows()):
        M[f'RotName{NUM_WORDS[i]}'] = rot_label(rw['topic_name'])
        M[f'RotPct{NUM_WORDS[i]}']  = pct1(rw['alpha_pct'])
        M[f'RotF{NUM_WORDS[i]}']    = f1(rw['f_stat_k'])
    # Cumulative share of net weight carried by the top five subjects.
    M['RotTopFiveCumPct'] = pct1(top5['alpha_pct'].sum())
    # Propaganda dominance: share of net weight on 'Propaganda *' subjects, and
    # how many of the ten highest-weight subjects are propaganda.
    is_prop = rot['topic_name'].astype(str).str.startswith('Propaganda')
    M['RotPropagandaPct'] = pct1(100 * rot.loc[is_prop, 'alpha'].sum())
    top10 = rot_pos.head(10)
    M['RotPropTopTen'] = big(int(top10['topic_name'].astype(str).str.startswith('Propaganda').sum()))
    M['RotNPositive']  = big(int((rot['alpha'] > 0).sum()))
except (FileNotFoundError, IndexError, KeyError):
    pass

# ── Multiple-testing correction over the primary executive family ─────────────
# Holm (FWER, any dependence) + Benjamini-Hochberg (FDR) over the committed
# 2SLS p-values; produced by code/03_estimation/07_multiplicity.R. Answers the
# "you tested many outcomes" objection: how many survive, and the adjusted p on
# the two headline findings (margin, mayoral blank rate).
try:
    mult = pd.read_csv(os.path.join(REG, 'multiplicity_adjusted.csv'))
    M['MultFamilyN'] = big(len(mult))
    M['MultRawSig']  = big(int(mult['sig_raw_5'].sum()))
    M['MultHolmSig'] = big(int(mult['sig_holm_5'].sum()))
    M['MultBHSig']   = big(int(mult['sig_bh_5'].sum()))
    mm = mult.set_index('outcome')
    M['MultMarginHolm'] = pval(mm.loc['delta_margin_top1_top2_2024_2020', 'p_holm'])
    M['MultMarginBH']   = pval(mm.loc['delta_margin_top1_top2_2024_2020', 'p_bh'])
...
with open(macros_path, 'w', encoding='utf-8') as f:
    f.write('% abstract_macros.tex — auto-generated by 06_abstract_macros.py\n')
...
def nopluscoef(c):
    """Table house style: positive coefficients carry NO leading '+'
    (the absence of a minus already reads positive); negatives keep '-';
    zero prints unsigned. The macros (\\MarginCoef etc.) keep their signed
    formatting because they are reused in prose — only the TABLE cells use this."""
    return c[1:] if c.startswith('+') else c

def row(label, c, s, p, mean, bold=False):
    """Build one table row. The coefficient cell drops the leading '+' to match
    the hand-built R tables' no-sign-on-positive house convention."""
    c = nopluscoef(c)
...
with open(table_path, 'w', encoding='utf-8') as f:
    f.write('% abstract_table.tex — auto-generated by 06_abstract_macros.py\n')
    f.write('% DO NOT EDIT MANUALLY.\n\n')

...
def main() -> None:
    ...
    for yr in YEARS:
        ...
        # Year-relative renewal / incumbency, anchored to the SAME SEAT
        # (municipality x office) in each cycle's OWN prior cycle
        # (2024->2020, 2020->2016, 2016->2012):
        #   is_new_vs_prior         : did NOT contest this seat in yr-4
        #   is_incumbent_from_prior : WON this seat in yr-4 (sitting incumbent)
        # Unlike the old fixed "vs 2020" anchor (which only existed for 2024),
        # every cycle carries a comparable measure, so a 2016 baseline exists.
        if prev_yr in YEARS:
            prev_seat = all_cands[all_cands["election_year"] == prev_yr]
            ran_seat = set(zip(prev_seat["municipality_id_tse"],
                               prev_seat["office_group"], prev_seat["person_key"]))
            won = prev_seat[prev_seat["is_elected"] == 1]
            won_seat = set(zip(won["municipality_id_tse"],
                               won["office_group"], won["person_key"]))
            seat_tup = list(zip(current["municipality_id_tse"],
                                current["office_group"], current["person_key"]))
            current["is_new_vs_prior"] = [int(t not in ran_seat) for t in seat_tup]
            current["is_incumbent_from_prior"] = [int(t in won_seat) for t in seat_tup]
        else:
            current["is_new_vs_prior"] = np.nan
            current["is_incumbent_from_prior"] = np.nan

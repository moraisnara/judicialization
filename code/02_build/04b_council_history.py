"""
Builds a COUNCIL (vereador) candidate experience panel from TSE consulta_cand
files (2012-2024), mirroring 04_candidate_history.py but for the legislative
race. Vereador has no term limit and is multi-seat, so there is no open-seat
concept; otherwise the entrant typology is identical.

For each candidate running for VEREADOR, computes at time of filing how many
times they have run before (for council) and how many times they won.
Aggregates to municipality x year.

Output: data/clean/council_experience_panel.csv
Columns: election_year, state, municipality_id_tse, municipality_name,
         share_first_time_candidates  -- never ran for vereador anywhere before
         mean_prior_candidacies       -- mean number of prior council runs
         share_prior_winners          -- share who previously won a seat
         share_career_politicians     -- share with 3+ prior council runs
         share_serial_challenger      -- ran in same muni last cycle, lost
         share_cross_cycle_returner   -- prior history but sat out last cycle
         n_candidates                 -- council candidates in the cell
"""
...
def load_year(year: int) -> pd.DataFrame:
    ...
    # Municipal elections (CD_TIPO_ELEICAO == "2"), council race, first round
    ...
    if "DS_CARGO" in df.columns:
        df = df[df["DS_CARGO"].str.upper().str.contains("VEREADOR", na=False)].copy()
...
def main() -> None:
    ...
    print("[1] Loading council candidate files 2012-2024", flush=True)
    all_frames = []
    for yr in YEARS:
        df = load_year(yr)
        if not df.empty:
            print(f"  {yr}: {len(df):,} council candidates", flush=True)
    ...
    # ------------------------------------------------------------------ #
    # [3] Serial challenger and cross-cycle returner flags (per candidate)
    # ------------------------------------------------------------------ #
    print("\n[3] Computing entrant typology flags", flush=True)
    flagged_results = []
    for yr in YEARS:
        ...
        # Cross-cycle returner: has any prior candidacy but did NOT run in yr-4
        if prev_yr in YEARS:
    ...
    print("\n[4] Aggregating to municipality x year", flush=True)
    panel = (
        all_with_flags
    ...
    out = DERIVED_DIR / "council_experience_panel.csv"
